...
class Article(models.Model):
    ...
    similar = models.TextField('Похожие статьи, id других статей через запятую', blank=True, null=True)
    description = models.TextField('Краткое описание статьи (200 символов)',max_length=200, blank=False, null=True)
    text = RichTextUploadingField('Статья.', blank=False, null=True)
    views = models.IntegerField('Просмотров', default=0)
    is_active = models.BooleanField('Отображать статью ?', default=False, db_index=True)
